...
class AsymmetricLaplace(continuous.Continuous):
    ...
    def random(self, point=None, size=None):
        ...
        if size is not None:
            ...
            x = -tt.log((1 - u) * (1 + self.symmetry ** 2)) / (
                self.symmetry * self.scale
            ) * (
                u > ((self.symmetry ** 2) / (1 + self.symmetry ** 2))
            ) + self.symmetry * tt.log(
                u * (1 + self.symmetry ** 2) / (self.symmetry ** 2)
            ) / self.scale * (
                u < ((self.symmetry ** 2) / (1 + self.symmetry ** 2))
            )
        ...
        if u > (self.symmetry ** 2) / (1 + self.symmetry ** 2):
            ...
            x = (
                self.symmetry
                * tt.log(u * (1 + self.symmetry ** 2) / (self.symmetry ** 2))
                / self.scale
            )
