...
def plot_intervention_effectiveness(
    posterior_samples,
    cm_names=None,
    intervention_varname='All_CM_Reductions',
    xlim="auto",
    newfig=True,
):
    if newfig:
    ...
    if isinstance(posterior_samples, dict):
        per_red = 100 * (1 - posterior_samples[intervention_varname])
    else:
        per_red = 100 * (1 - posterior_samples)
    ...
    if cm_names is None:
        if 'cm_names' in posterior_samples.keys():
            cm_names = posterior_samples['cm_names']
        else:
