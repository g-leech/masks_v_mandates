...
if __name__ == "__main__":
    ...
    region_indices = np.random.choice(np.arange(masks_object.nRs), size=masks_object.nRs, replace=True)
    print(f"Boostrapped: {np.unique(region_indices).size} unique regions")

    data.Rs = np.array(data.Rs)[region_indices].tolist()
    data.NewCases = data.NewCases[region_indices, :]
    data.ActiveCMs = data.ActiveCMs[region_indices, :, :]
