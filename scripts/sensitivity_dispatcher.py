...
def run_types_to_commands(run_types, exp_options):
    ...
    for config in configs:
        ...
        for rt in run_types:
            ...
            cmds = [
                f"python sensitivity_analysis/{experiment_file} --model_type {model_type}"
                    ...
                    for c in cmds:
                        for v in value:
                            if isinstance(v, list):
                                ...
                                final_cmd = f"{new_cmd}"
                                new_cmds.append(final_cmd)
                            else:
                                print('v is not a list')
                                print(f'{v}')
                                new_cmd = f"{c} --{key} {v}"
...
if __name__ == "__main__":
    ...
    with open("sensitivity_analysis/sensitivity_analysis.yaml", "r") as stream:
        try:
